...
| 6 | Automated Test Suite & CI | Equivalence tests against the Solidity reference, wired into CI so future changes are regression-tested. | 100 | Local-devnet equivalence harness (anvil for the Solidity reference, dev-node+eth-rpc for the native runtime), 7 circuit shapes x 5 test vectors = 35/35 passing locally (25/25 confirmed on real GitHub Actions for the original 5-shape matrix; the 2 newest shapes validated locally and, for one, on real Paseo, pending a fresh CI run with all 7). **Proof:** [`06_automated_test_suite_ci.md`](./06_automated_test_suite_ci.md). |
