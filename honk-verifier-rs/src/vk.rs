...
fn fq(hex64: &str) -> Fq {
    assert_eq!(hex64.len(), 64, "expected 64-char hex string");
...
/// G2 generator and KZG SRS point (hardcoded from HonkVerifier.sol).
/// EVM pairing precompile format: (x_imag, x_real, y_imag, y_real) → Fq2::new(c0=real, c1=imag)
pub fn g2_generator() -> G2Affine {
    G2Affine::new_unchecked(
        Fq2::new(
            fq("1800deef121f1e76426a00665e5c4479674322d4f75edadd46debd5cd992f6ed"), // x.c0 (real)
            fq("198e9393920d483a7260bfb731fb5d25f1aa493335a9e71297e485b7aef312c2"), // x.c1 (imag)
        ),
        Fq2::new(
            fq("12c85ea5db8c6deb4aab71808dcb408fe3d1e7690c43d37b4ce6cc0166fa7daa"), // y.c0 (real)
            fq("090689d0585ff075ec9e99ad690c3395bc4b313370b38ef355acdadcd122975b"), // y.c1 (imag)
        ),
    )
}

pub fn g2_kzg_srs() -> G2Affine {
    G2Affine::new_unchecked(
        Fq2::new(
            fq("0118c4d5b837bcc2bc89b5b398b5974e9f5944073b32078b7e231fec938883b0"), // x.c0 (real)
            fq("260e01b251f6f1c7e7ff4e580791dee8ea51d87a358e038b4efe30fac09383c1"), // x.c1 (imag)
        ),
        Fq2::new(
            fq("22febda3c0c0632a56475b4214e5615e11e6dd3f96e6cea2854a87d4dacc5e55"), // y.c0 (real)
            fq("04fc6369f7110fe3d25156c1bb9a72859cf2a04641f99ba4ee413c80da6a5fe4"), // y.c1 (imag)
        ),
    )
}
